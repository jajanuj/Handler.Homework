...
namespace ArtEQ
{
    public partial class ucRecipeManager : UserControl
    {
        ...
        public void UpdateControls()
        {
            ...
            comTextMapRecipePath.Text = strViewDirectory = Path.GetDirectoryName(comTextInspRecipe.Text);
            if (Directory.Exists(strViewDirectory) == true)
            {
                List<string> m_listProductName = new List<string>();
                foreach (var sFileName in Directory.GetFiles(strViewDirectory))
                {
                ...
                for (int i = 0; i < m_listProductName.Count; i++)
                {
                    ...
                    dataGridView1[RecipeName.Index, i].Value = Path.GetFileName(m_listProductName[i]).Replace(".ini", "");
        ...
        public void _ShowForm(bool Dialog = true)
        {
            ...
            Size = new System.Drawing.Size(654, 512);
            mForm.Size = new Size(Size.Width + 16, Size.Height + 39);
            ...
            if (strSelectRecipe == string.Empty)
            {
                bool isEmpty = dataGridView1.Rows.Count == 0 || (dataGridView1.AllowUserToAddRows && dataGridView1.Rows.Count == 1);
                if (!isEmpty)
                {
                    strSelectRecipe = dataGridView1[RecipeName.Index, 0]?.Value.ToString();
                }
            }
        ...
        {
            clsLog.Log(clsEnum.enuLogName.SystemLog, clsCmData.g_strNowUser + " " + Name + " - " + ((Control)sender).Name);
        ...
        private void btnAddProduct_Click(object sender, EventArgs e)
        {
            ...
            if (clsDialogShow.InputString("Create New Recipe", "New Recipe Name", ref sNewFileName) == DialogResult.OK)
            {
                ...
                {
                    clsLog.Log(clsEnum.enuLogName.SystemLog, clsCmData.g_strNowUser + " " + Name + " - " + ((Control)sender).Name + ", Create New Recipe Path : " + sPath);
                    File.Create(sPath);
                    UpdateControls();
                }
                else
        ...
        private void btnLoadProduct_Click(object sender, EventArgs e)
        {
            ...
            {
                clsLog.Log(clsEnum.enuLogName.SystemLog, clsCmData.g_strNowUser + " " + Name + " - " + ((Control)sender).Name + ", Load Recipe Path : " + sPath);
        ...
        private void btnDelProduct_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(sPath) == true)
            {
                if (formMessageBox.Show("Are You Sure Want To Delete?", "Delete Recipe", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        clsLog.Log(clsEnum.enuLogName.SystemLog, clsCmData.g_strNowUser + " " + Name + " - " + ((Control)sender).Name + ", Delete Recipe Path : " + sPath);
                        File.Delete(sPath);
                        if (Directory.Exists(sPath.Replace(".ini", "")) == true)//@1.0.0.47-12@
                        {
                            Directory.Delete(sPath.Replace(".ini", ""), true);//@1.0.0.37-13@
                        }
                        _inUes = false;
                        UpdateControls();
                    }
        ...
        private void btnCopyProduct_Click(object sender, EventArgs e)
        {
            ...
            if (clsDialogShow.InputString("Copy New Recipe", "Copy Recipe Name", ref sNewFileName) == DialogResult.OK)
            {
                ...
                if (!File.Exists(sPathTarget))
                {
                    clsLog.Log(clsEnum.enuLogName.SystemLog, $"{clsCmData.g_strNowUser} {Name} - {((Control)sender).Name}, Copy From Recipe Path : {sPathSource}");
                    clsLog.Log(clsEnum.enuLogName.SystemLog, $"{clsCmData.g_strNowUser} {Name} - {((Control)sender).Name}, Copy New Recipe Path : {sPathTarget}");
                    if (_CopyRecipe(sPathSource, sPathTarget) == false)
                    {
                        clsLog.Log(clsEnum.enuLogName.SystemLog, $"{clsCmData.g_strNowUser} {Name} - {((Control)sender).Name}, Copy Recipe Error");
        ...
        private void mForm_SizeChanged(object sender, EventArgs e)
        {
            ...
            {
                GetSingleton().Size = GetSingleton().Parent.ClientSize;
            }
        }
