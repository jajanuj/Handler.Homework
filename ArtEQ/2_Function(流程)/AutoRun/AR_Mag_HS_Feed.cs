        ...
        protected override void Scenario()
        {
            switch (iStepIndex)
            {
                ...
                case 201000:
                    ...
                    {
                        clsLog.Log(clsEnum.enuLogName.ProcessLog.ToString(), $"{strThreadLogName} : {Mag_HS_Feed().m_enuAction.ToString()} : Load Done → 回閒置等處理");
                        iStepIndex = 100000;
                    ...
                    {
                        clsLog.Log(clsEnum.enuLogName.ProcessLog.ToString(), $"{strThreadLogName} : {Mag_HS_Feed().m_enuAction.ToString()} : Load Fail → 回閒置等處理");
                        iStepIndex = 100000;
                        if (CheckNextSlotNo() < 0)
                        {
