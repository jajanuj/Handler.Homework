...
using ArtSystem;
using ArtTeach;
using static ArtProcModuleLib.clsInfoBase;
using ArtEQ;


namespace ArtEQ
{
    public abstract class BaseMagazing : clsThreadProc
    {
        #region //===================== Axis / Pos / IO =====================
        ...
        protected int m_iHomeDelay = 1000;

        protected clsEnum.enuAxis m_P_Z;

        protected clsEnum.enuPosName m_posSlot1_Z;

        protected clsEnum.enuDi m_DI_Magazine_01_Present;
        //protected clsEnum.enuDi m_DI_Magazine_02_Present;
        //protected clsEnum.enuDi m_DI_Magazine_03_Present;
        //protected clsEnum.enuDi m_DI_Magazine_04_Present;
        //protected clsEnum.enuDi m_DI_Magazine_05_Present;
        ...
        protected clsBoxMotion m_P_Z_Motion = new clsBoxMotion();
        ...
        public enum enuAction
        {
            ...
            // ── 推料給下游（Unload：Magazine → Setup Area）──
            Magazine_Load = 20000,
            Magazine_Load_Waiting,  // 等待下游 Setup Area 淨空
            Magazine_Loading,       // 推桿推出中
            Magazine_Load_Done,
            Magazine_Load_Fail,

            // ── 收料回 Magazine（Load：Unload Lane → Magazine）──
            Magazine_Unload = 30000,
            Magazine_Unload_Waiting,    // 等待上游送來 Boat
            Magazine_Unloading,         // 等待 Boat 進入 Slot
        ...
        /// <summary>
        /// Magazine 每一格 Slot 的帳。
        /// </summary>
        public clsMagazineInfo m_MagazineInfo = new clsMagazineInfo();

        /// <summary>
        /// true：帳已經複製給下游，但還沒收到下游 Load_Done ACK。
        /// 這時 Magazine Slot 帳不能再被推出。
        /// </summary>
        ...
        protected BaseMagazing(string p_strName) : base(p_strName)
        {
        ...
        protected override void Scenario()
        {
            switch (iStepIndex)
            {
                ...
                case 50:
                    ...
                    clsLog.Log(clsEnum.enuLogName.ProcessLog.ToString(), strThreadLogName + " : " + m_enuAction.ToString());
                    break;
                #endregion
                ...
                case 10200:
                    // 【檢查 Magazine 在席】確認 Magazine 是否已放置到位
                    // 模擬模式：強制設定 Magazine 在席信號為 true
                    // 實際模式：讀取 DI 信號判斷
                    #region 確認 Magazine 在席
                    if (b_Simulation())
                    {
                        SetDi(m_DI_Magazine_01_Present, true);
                        iStepIndex = 10300; // Magazine 在席，繼續初始化
                    }
                    else if (GetDi(m_DI_Magazine_01_Present))
                    {
                        iStepIndex = 10300; // Magazine 在席，繼續初始化
                    }
                    else
                    {
                        // Magazine 不在席，報警並跳到失敗流程
                        clsEditRunThread.ReportAlarm(clsEnum.enuAlarm.Need_Empty_Magazine_To_Collect_Boat);
                        iStepIndex = 10998;
                    }
                    #endregion
                    break;
                ...
                case 10300:
                    ...
                    m_Cylinder_OutPush.Initial(
                        ...
                        m_DI_OutPush_Retract, true, // 推桿縮回 DI (常閉)
                        m_DI_OutPush_Extend, true,  // 推桿伸出 DI (常閉)
                ...
                #region //===================Magazine_Load (推料給下游) (20000~29999)===================
                case 20000:
                    // 【推料流程開始】進入推料等待狀態
                ...
                case 20001:
                    ...
                    {
                        // Magazine 不在席，報警
                        clsEditRunThread.ReportAlarm(
                            clsEnum.enuAlarm.Need_Empty_Magazine_To_Collect_Boat);
                        iStepIndex = 20998;
                    }
                    #endregion
                    break;
                ...
                case 20130:
                    // 【等待下游淨空】確認下游 Setup Area 已淨空，可以接收 Tray
                    // 若下游未淨空，維持等待狀態
                    // 若下游已淨空，進入推料中狀態
                    #region 等待下游 Area 淨空
                    m_enuAction = enuAction.Magazine_Load_Waiting;
                ...
                case 20310:
                    ...
                    m_CtrlBox.Action(ref iStepIndex, 20999, 20998);
                ...
                case 20999:
                    // 【推料完成】執行帳料轉移（Magazine → 下游），設定狀態為完成
                    TransferBillAfterUnloading(); // 帳料從 Magazine 轉移給下游
                    m_enuAction = enuAction.Magazine_Load_Done;
                    iStepIndex = 999;
                    break;
                #endregion

                #region //===================Magazine_Unload (收料回 Magazine) (30000~39999)===================
                case 30000:
                    // 【收料流程開始】進入收料等待狀態
                ...
                case 30001:
                    // 【檢查 Magazine 空位】確認 Magazine 在席且有空位可收料
                    // 模擬模式：強制設定 Magazine 不在席（模擬有空位）
                    // 實際模式：讀取 DI 判斷（Slot 空 = Present 為 false）
                    #region 確認 Magazine 在席且有空位
                    if (b_Simulation())
                    {
                        SetDi(m_DI_Magazine_01_Present, false);
                        iStepIndex = 30100;
                    }
                    if (!GetDi(m_DI_Magazine_01_Present))
                    {
                        iStepIndex = 30100; // Slot 有空位，繼續收料流程
                    }
                    else
                    {
                        // Slot 已滿，報警
                ...
                case 30120:
                    // 【等待上游送料】等待上游 Unload Lane 送來 Tray
                    // 若上游尚未送料，維持等待狀態
                    // 若上游已送料，進入收料中狀態
                    #region 等待上游 Unload Lane 送來 Tray
                    if (ReadyToLoad())
                    {
                ...
                case 30130:
                    ...
                    {
                        // 超時仍未偵測到 Tray，報警
                        clsEditRunThread.ReportAlarm(clsEnum.enuAlarm.Cylinder_Timeout_Lane_Unload);
                        iStepIndex = 30998;
                    }
                    #endregion
                    break;
        ...
        {
            clsLog.Log(clsEnum.enuLogName.ProcessLog, strThreadLogName + " : " + msg);
        }
        ...
        protected double CalculateSlotPosZ(int p_iSlotNo)
        {
            ...
            double pitch = GetPmt(clsEnum.enuPmtName.Rec_Load_Magzine_Pitch);
            return firstSlotPos + (p_iSlotNo - 1) * pitch;
        }

        protected virtual bool CheckStatus() { return false; }

        #region //===================== 帳料方法 =====================

        /// <summary>
        /// 是否啟用帳控。
        /// 預設 false，避免影響其他尚未導入帳控的 Magazine。
        /// 子類別要用時 override return true。
        /// </summary>
        protected virtual bool UseBillControl()
        {
            return false;
        }
        ...
        /// </summary>
        protected void EnsureMagazineBill()
        {
            ...
            if (m_MagazineInfo.m_trayInfo == null)
                m_MagazineInfo.m_trayInfo = new System.Collections.Generic.Dictionary<int, clsTrayInfo>();

            // ========== 第三段：移除超出使用範圍的 Slot ==========
            // 只保留目前使用者設定的 Slot 1 ~ m_iUseSlotCount。
            // 例如：使用者設定只用 3 個 Slot，就不應該保留 Slot 4、Slot 5 的帳。
            // 建立待移除的 Slot 清單
            System.Collections.Generic.List<int> removeKeys = new System.Collections.Generic.List<int>();
        ...
        {
            // 推桿氣缸初始化
            m_Cylinder_OutPush.Initial(m_DO_OutPush_Extend, m_DI_OutPush_Retract, true, m_DI_OutPush_Extend, true, (uint)m_iPutterTimeout, "Putter_HomeAlarm", "Putter_ReachAlarm");

            // Z 軸初始化
            m_P_Z_Motion.Initial(m_P_Z, MotionUnit.Minimeter, 10000, "Z-Axis Timeout");
            m_P_Z_Motion.iSimulateDelayTime = 500;
            clsMotionCtrl.SetServo(m_P_Z, true);
        }
        ...
        /// <summary> 推料給下游 Setup Area </summary>
        ...
        public bool DI_MagExist() => clsDioCtrl.GetDi(m_DI_Magazine_01_Present);
        ...
        protected void InitialMagazineBill()
        {
            ...
            clsLog.Log(
                clsEnum.enuLogName.ProcessLog.ToString(),
                $"{strThreadLogName} : Initial Magazine Bill. SlotCount={m_iUseSlotCount}");
        }
        ...
        private int m_iPutterTimeout => GetPmt(clsEnum.enuPmtName.Sys_Timeout_Putter);
        private int m_iPutterBeforeDelay => GetPmt(clsEnum.enuPmtName.Sys_Delay_Putter_Before);
        private int m_iPutterAfterDelay => GetPmt(clsEnum.enuPmtName.Sys_Delay_Putter_After);
