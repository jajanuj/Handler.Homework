...
namespace ArtEQ
{
    ...
    public partial class formLogin : Form
    {
        ...
        public bool _ShowLoginForm()
        {
            ...
            //palPassWD.Enabled = clsArtSystem.bIsSoftwareSimulate == true;
        ...
        public bool _Login(string UserName, string Password)
        {
            ...
            else if ((UserName == "ArtAE" && Password == "076071828")//TQIC規範
                || UserName == "ArtQC")
            {
                clsCmData.g_strNowUser = UserName;
            ...
            {
                //寫入Log
                clsLog.Log(clsEnum.enuLogName.LoginLog, "[Login (LV" + clsCmData.g_iNowUserLevel + ")] User Name : " + clsCmData.g_strNowUser);
                ...
                if (clsArtSystem.bIsSoftwareSimulate == true
                    && clsCmData.g_iNowUserLevel == 0)
                {
                    #region//Level1 可以使用所有權限
                    List<string> LstFunc = ucUserAccount.GetFuncList();
                    bool bChangeSetting = false;
                    foreach (string sFunc in LstFunc)
                    {
                        if (sFunc == "Alarm Manage"
                            || sFunc == "Multi System")
                        {
                            for (int i = 1; i < 10; i++)
                            {
                                ucUserAccount.SetFuncEnable(i, sFunc, false); //@1023@
                            }
                        }
                        else if (ucUserAccount.GetSingleton().IsSubFuncUsable(sFunc) == false)
                        {
                            ucUserAccount.SetFuncEnable(1, sFunc, true);
                            bChangeSetting = true;
                        }
                    }
                    if (bChangeSetting == true)
                    {
                        ucUserAccount.GetSingleton().UpdateControls();
                    }
                    #endregion
        ...
        private void palPassWD_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            //        && iMouseLeftCount == 2)
            //    {
            //        //clsLog.Log(clsEnum.enuLogName.LoginLog, "[" + this.Name + "] AE Account Set.");
            //        textBoxUserName.Text = "ArtAE";
            //        textBoxPassWord.Text = "076071828";
            //        iMouseRightCount = 0;
            //        iMouseLeftCount = 0;
