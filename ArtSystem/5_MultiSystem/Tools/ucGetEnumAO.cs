...
{
    public partial class ucGetEnumAO : ucBaseUserControl
    {
        #region //=====================  區域變數設置 =====================
        ...
        static private ucGetEnumAO m_Singleton;
        /// <summary> 取得唯一物件，避免重覆設置  </summary>
        static public ucGetEnumAO GetSingleton()
        {
            if (m_Singleton == null)
            {
                m_Singleton = new ucGetEnumAO();
            }
            return m_Singleton;
        }
        ...
        public DialogResult _ShowFormDialog(ref clsEnum.enuDo? p_enuDO, bool AllEnum = false)
        {
            ...
            foreach (clsEnum.enuDo eDO in Enum.GetValues(typeof(clsEnum.enuDo)))
            {
                if (AllEnum == false)
                {
                    if (clsDioMotion.mLst_AOEnum.Contains(eDO) == false)
                    {
                        continue;
                    }
                }
            ...
            if(p_enuDO != null)
            {
                string sDIItem = p_enuDO.ToString().Replace("DO","AO");
                if (AllEnum == false)
                {
                    if (DicDoName.ContainsKey((clsEnum.enuDo)p_enuDO) == true)
                    {
                        sDIItem += "-(" + DicDoName[(clsEnum.enuDo)p_enuDO] + ")";
                    }
                }
            ...
            mForm.Text = clsLanguage.GetTranslation("Get AI Enum");
