...
{
    public partial class ucGetEnumAI : ucBaseUserControl
    {
        #region //=====================  區域變數設置 =====================
        ...
        static private ucGetEnumAI m_Singleton;
        /// <summary> 取得唯一物件，避免重覆設置  </summary>
        static public ucGetEnumAI GetSingleton()
        {
            if (m_Singleton == null)
            {
                m_Singleton = new ucGetEnumAI();
            }
            return m_Singleton;
        }
        ...
        public DialogResult _ShowFormDialog(ref clsEnum.enuDi? p_enuDI, bool AllEnum = false)
        {
            ...
            foreach (clsEnum.enuDi eDI in Enum.GetValues(typeof(clsEnum.enuDi)))
            {
                if (AllEnum == false)
                {
                    if (clsDioMotion.mLst_AIEnum.Contains(eDI) == false)
                    {
                        continue;
                    }
                }
            ...
            if(p_enuDI != null)
            {
                string sDIItem = p_enuDI.ToString().Replace("DI","AI");
                if (AllEnum == false)
                {
